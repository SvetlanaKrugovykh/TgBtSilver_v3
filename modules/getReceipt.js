...
async function getReceipt(
	...
	try {
    let query = `Execute;GetReceipt;${telNumber};КОНЕЦ`
    if (condition === 'akt') query = `Execute;__GetAkt__;${telNumber};КОНЕЦ`
		...
		if (!response.status == 200) {
			...
			if (!response.status == 200) {
				...
				try {
					...
					setTimeout(function () {
						bot.sendDocument(msg.chat.id, fileFullName).catch(function (error) {
							logWithTime(error)
						})
					}, 1111)
				} catch (err) {
